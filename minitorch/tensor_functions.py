...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        t = grad_output.f.add_zip(tensor([1.0]), grad_output.f.neg_map(sigmoid))
        t = grad_output.f.mul_zip(t, sigmoid)
...
class LT(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        return grad_output.zeros(b_shape), grad_output.zeros(a_shape)
...
class Permute(Function):
    ...
    def forward(ctx: Context, a: Tensor, order: Tensor) -> Tensor:
        ...
        return a._new(a._tensor.permute(*order.tuple()[0].astype(np.int32)), )

    @staticmethod
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        (order, ) = ctx.saved_tensors
        new_order = [i[0] for i in sorted(enumerate(order.tuple()[0].astype(np.int32)), key=lambda x: x[1])]
